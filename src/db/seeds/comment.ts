...
import { CommentSchemaType } from "@/db/schema/blog/comment";
...
const parentCommentsMock = async () => {
	...
	const data: CommentSchemaType[] = randomPosts.map((post) => ({
...
const childCommentsMock = async () => {
	...
	const data: CommentSchemaType[] = randomComments.map((comment) => ({
		content: faker.lorem.sentences(),
